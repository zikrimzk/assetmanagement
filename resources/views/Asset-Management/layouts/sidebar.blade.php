 <!-- Start::app-sidebar -->
 <aside class="app-sidebar sticky shadow-sm donotprint" id="sidebar">
     <div class="main-sidebar" id="sidebar-scroll">
  
         <nav class="main-menu-container nav nav-pills flex-column sub-open">
             <div class="slide-left" id="slide-left">
                 <svg xmlns="http://www.w3.org/2000/svg" fill="#7b8191" width="24" height="24"
                     viewBox="0 0 24 24">
                     <path d="M13.293 6.293 7.586 12l5.707 5.707 1.414-1.414L10.414 12l4.293-4.293z"></path>
                 </svg>
             </div>
             <ul class="main-menu">
                
                 <li class="slide__category"><span class="category-name">Main</span></li>

                 @if (Auth::user()->staff_role == 1)
                     <li class="slide">
                         <a href="{{ route('user-index') }}" class="side-menu__item">

                             <i class='bx bxs-home side-menu__icon'></i>
                             <span class="side-menu__label">Dashboards</span>

                         </a>
                     </li>
                 @else
                     <li class="slide">
                         <a href="{{ route('user-index') }}" class="side-menu__item">

                             <i class='bx bxs-home side-menu__icon'></i>
                             <span class="side-menu__label">Home</span>

                         </a>
                     </li>
                 @endif

                 @if (Auth::user()->staff_role != 5)
                     
                     <li class="slide__category"><span class="category-name">Management</span></li>
                    
                     <li class="slide">
                         <a href="{{ route('asset-index') }}" class="side-menu__item">
                             <i class='bx bx-layer side-menu__icon'></i>
                             <span class="side-menu__label">Asset Management</span>

                         </a>
                     </li>
              
                 @endif

                 @if (Auth::user()->staff_role == 1)
                    
                     <li class="slide__category"><span class="category-name">System
                             Setting</span></li>
            
                     <li class="slide has-sub">
                         <a href="javascript:void(0);" class="side-menu__item">
                             <i class='bx bxs-business side-menu__icon'></i>
                             <span class="side-menu__label">Company</span>
                             <i class="fe fe-chevron-down side-menu__angle"></i>
                         </a>
                         <ul class="slide-menu child1">
                             <li class="slide side-menu__label1">
                                 <a href="javascript:void(0)">Company</a>
                             </li>
                             <li class="slide">
                                 <a href="{{ route('company-index') }}" class="side-menu__item">Company Management</a>
                             </li>
                             <li class="slide">
                                 <a href="{{ route('area-index') }}" class="side-menu__item">Area Management</a>
                             </li>
                         </ul>
                     </li>
                
                     <li class="slide has-sub">
                         <a href="javascript:void(0);" class="side-menu__item">
                             <i class='bx bxs-user-account side-menu__icon'></i>
                             <span class="side-menu__label">Staff</span>
                             <i class="fe fe-chevron-down side-menu__angle"></i>
                         </a>
                         <ul class="slide-menu child1">
                             <li class="slide side-menu__label1">
                                 <a href="javascript:void(0)">Staff</a>
                             </li>
                             <li class="slide">
                                 <a href="{{ route('department-index') }}" class="side-menu__item">Department
                                     Management</a>
                             </li>
                             <li class="slide">
                                 <a href="{{ route('staff-index') }}" class="side-menu__item">Staff Management</a>
                             </li>
                         </ul>
                     </li>
                 @endif
             </ul>
             <div class="slide-right" id="slide-right"><svg xmlns="http://www.w3.org/2000/svg" fill="#7b8191"
                     width="24" height="24" viewBox="0 0 24 24">
                     <path d="M10.707 17.707 16.414 12l-5.707-5.707-1.414 1.414L13.586 12l-4.293 4.293z"></path>
                 </svg></div>
         </nav>
     </div>
 </aside>
 <!-- End::app-sidebar -->
